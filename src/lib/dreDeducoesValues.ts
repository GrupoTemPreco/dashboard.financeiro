...
/** Mesmos prefixos/segmentos do grupo "Imposto sobre venda" em DespesasOperacionaisTable */
const DEDUCOES_LEAVES = [
  {
    id: 'deducoes-simples',
    name: 'Simples Nacional pago no período (R$)',
    chartOfAccountsPrefix: '03.1',
    chartOfAccountsSegmentContains: 'Simples Nacional'
  },
  {
    id: 'deducoes-icms',
    name: 'ICMS Pago No Período',
    chartOfAccountsPrefix: '03.2',
    chartOfAccountsSegmentContains: 'ICMS'
  },
  {
    id: 'deducoes-parcelamento',
    name: 'Parcelamento de Divida Ativa pago no período',
    chartOfAccountsPrefix: '03.3',
    chartOfAccountsSegmentContains: 'Parcelamento'
  }
] as const;

export type DreDeducaoCellValues = {
...
/**
 * Deduções da DRE: mesma base que Despesas Operacionais (contas a pagar, status, período anterior/atual, filtros globais).
 * Chaves: deducoes-simples, deducoes-icms, deducoes-parcelamento, deducoes (total).
 */
export function computeDeducoesValuesMap(
  accountsPayable: any[],
  filters: { startDate: string; endDate: string; groups: string[]; companies: string[] },
  companies: any[],
  capMatchCollector?: CapCoaMatchCollector
): Record<string, DreDeducaoCellValues> {
  ...
  const matchApToAccount = (ap: any, account: (typeof DEDUCOES_LEAVES)[number]): boolean => {
  ...
  const sumFromList = (list: any[], account: (typeof DEDUCOES_LEAVES)[number]): number =>
    list.filter(ap => matchApToAccount(ap, account)).reduce((sum, ap) => sum + Math.abs(parseFloat(ap.amount || 0)), 0);

  const map: Record<string, DreDeducaoCellValues> = {};
  const ZERO: DreDeducaoCellValues = { prevRealizado: 0, curRealizado: 0, curPrevisto: 0 };

  for (const account of DEDUCOES_LEAVES) {
    map[account.id] = {
  ...
  map['deducoes'] = DEDUCOES_LEAVES.reduce(
    (acc, leaf) => {
